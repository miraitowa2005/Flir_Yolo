...
from models.yolo_test import Model    # 第28行
from models.common import LightIENet, PixelGateController   #29line
...
def train(hyp, opt, device, tb_writer=None):
    ...
    if pretrained:
        ...
        # 创建LightIENet（仅PixelGate模式）

    ie_net = None
    if 'pixelgate' in opt.cfg.lower():
        ie_net = LightIENet().to(device)
        logger.info('PixelGate模式：LightIENet已创建')
    ...
    if ie_net is not None:
        optimizer.add_param_group({'params': ie_net.parameters(), 'weight_decay': 0.0})
        logger.info('LightIENet参数已加入优化器')
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # 提取RGB（前3通道），更新PixelGate的alpha_map
            imgs_rgb = imgs[:, :3]
            if ie_net is not None:
                ie_net.train()
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                if ie_net is not None and PixelGateController._alpha_map is not None:
                    alpha = PixelGateController._alpha_map
                    brightness = imgs_rgb.mean(dim=1, keepdim=True)
                    illum_label = F.interpolate(brightness, size=(80, 80), mode='bilinear', align_corners=False)
                    L_aux = F.mse_loss(alpha, illum_label)
                    dx = (alpha[:, :, :, 1:] - alpha[:, :, :, :-1]).abs().mean()
                    dy = (alpha[:, :, 1:, :] - alpha[:, :, :-1, :]).abs().mean()
                    L_smooth = dx + dy
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='D:/CatalogForProj/CV/ccf_c/weights/yolov5s.pt', help='initial weights path')
    parser.add_argument('--cfg', type=str, default='D:/CatalogForProj/CV/ccf_c/configs/models/yolov5s_pixelgate_FLIR_aligned.yaml', help='model.yaml path')
    ...
    parser.add_argument('--name', default='baseline_add_100epoch_4batch', help='save to project/name')
