...
# ============================================================
# PixelGate: 像素级光照感知融合模块
# ============================================================

class LightIENet(nn.Module):
    """
    轻量级光照估计网络
    输入: RGB图像 [B, 3, H, W]
    输出: 空间光照置信度图 alpha_map [B, 1, 80, 80]
    """
    def __init__(self):
        super(LightIENet, self).__init__()
        # Encoder
        self.enc1 = nn.Sequential(
            nn.Conv2d(3, 16, 3, stride=2, padding=1),
            nn.BatchNorm2d(16), nn.ReLU(inplace=True))   # [B,16,320,320]
        self.enc2 = nn.Sequential(
            nn.Conv2d(16, 32, 3, stride=2, padding=1),
            nn.BatchNorm2d(32), nn.ReLU(inplace=True))   # [B,32,160,160]
        self.enc3 = nn.Sequential(
            nn.Conv2d(32, 64, 3, stride=2, padding=1),
            nn.BatchNorm2d(64), nn.ReLU(inplace=True))   # [B,64,80,80]
        self.enc4 = nn.Sequential(
            nn.Conv2d(64, 128, 3, stride=2, padding=1),
            nn.BatchNorm2d(128), nn.ReLU(inplace=True))  # [B,128,40,40]
        self.enc5 = nn.Sequential(
            nn.Conv2d(128, 128, 3, stride=2, padding=1),
            nn.BatchNorm2d(128), nn.ReLU(inplace=True))  # [B,128,20,20]
        # Decoder
        self.dec1 = nn.Sequential(
            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),
            nn.Conv2d(128, 64, 3, padding=1),
            nn.BatchNorm2d(64), nn.ReLU(inplace=True))   # [B,64,40,40]
        self.dec2 = nn.Sequential(
            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),
            nn.Conv2d(64, 32, 3, padding=1),
            nn.BatchNorm2d(32), nn.ReLU(inplace=True))   # [B,32,80,80]
        # 输出
    ...
    def forward(self, x):
        ...
        x = self.dec1(x)
        x = self.dec2(x)
        return self.sigmoid(self.out_conv(x))  # [B, 1, 80, 80]


class PixelGateController:
    """
    全局控制器：存储LightIENet输出的alpha_map
    train.py里调用 update()，PixelGate.forward里调用 get_alpha()
    """
    _alpha_map = None  # [B, 1, 80, 80]

    @classmethod
    def update(cls, rgb_img, ie_net):
